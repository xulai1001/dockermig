...
class MigrateService(pyjsonrpc.HttpRequestHandler):
    ...
    def lazy_restore(self, client_ip, container):
        ...
        bundle_path = base_path + container + "/bundle/"
        with pushd(bundle_path):
            os.system("gnome-terminal -t "CRIU lazy-pages"-- criu lazy-pages --page-server --address %s --port 27000 -vv -D checkpoint -W checkpoint" % client_ip)
            run_cmd_timed("gnome-terminal -t "Container - %s" -- /usr/local/sbin/runc restore --image-path checkpoint --work-path checkpoint --lazy-pages %s" % (container, container))

#            run_cmd_timed("gnome-terminal -- /usr/local/sbin/runc restore --tcp-established --image-path checkpoint --work-path checkpoint --lazy-pages %s" % container)            
