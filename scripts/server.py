...
class MigrateService(pyjsonrpc.HttpRequestHandler):
    ...
    def restore(self, container):
        ...
        with pushd(bundle_path):
            ...
            run_cmd_timed("gnome-terminal -- /home/islab/src/dockermig/scripts/run.sh runc restore --debug --tcp-established --shell-job --file-locks --image-path checkpoint --work-path checkpoint --bundle bundle %s" % container)
        return retvar
...
#            time.sleep(1)
            ...
            run_cmd_timed("gnome-terminal -t 'Container - %s' -- /home/islab/src/dockermig/scripts/run.sh runc restore --debug --tcp-established --shell-job --file-locks --image-path checkpoint --work-path checkpoint --bundle bundle --lazy-pages %s" % (container, container))
        return retvar
